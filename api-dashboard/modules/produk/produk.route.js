...
async function routes(fastify, opts) {
  fastify.post("/produk", {schema: post}, async (req, reply) => {
    try {
      const { kode, name, price, categori, description, tags, stock, size } =
        req.body;
      const photos = {
  ...
  fastify.get("/produk", {schema: get}, async (req, reply) => {
    try {
      ...
      //Filter New Release
      const filterNewRelease = {}
      if(req.query.statusNewRealease){
        filterNewRelease.newRealease = req.query.statusNewRealease
      }
  ...
  fastify.put("/produk/:id", {schema: put}, async (req, reply) => {
    try {
      const { kode, name, price, categori, description, tags, status, stock, size, statusNewRealease } =
        req.body;
      const photos = {
        p1: req.body.photos.p1,
        p2: req.body.photos.p2,
